...
export const SHARED_UTILITIES = {
    ...
    date: {
        ...
        parseNaturalLanguageDate: (dateExpression) => {
            if (!dateExpression)
                return null;
            const now = new Date();
    ...
    api: {
        ...
        customers: {
            ...
            search: async (searchTerm, take = 1000) => {
                return await callIMPApi("/api/customer_list", {
                    search: searchTerm,
                    take
                });
        ...
        tasks: {
            ...
            getDetailsByCustomNumber: async (customNumber) => {
                return await callIMPApi("/api/task_details", {
        ...
        analytics: {
            ...
            getCustomerData: async (customerId, params = {}) => {
...
export async function callIMPApi(endpoint, additionalParams = {}) {
    // Rate limiting check
    const now = Date.now();
...
export function registerUtilityTools(server) {
    ...
    server.tool("date-utility", "Perform date calculations, validation, and formatting operations. Use 'parse' operation for natural language expressions like 'last 30 days', 'this month', 'Q1 2025'", {
        ...
        try {
            ...
            const interpretDateExpression = (expression) => {
                const now = new Date();
                const currentYear = now.getFullYear();
                const currentMonth = now.getMonth();
                ...
                // Check for exact matches first
                if (expr in dateInterpretation) {
                    return dateInterpretation[expr];
                }
                // Handle "last X days/weeks/months" patterns
                if (expr.includes('last') && expr.includes('days')) {
            ...
            const getNextQuarterRange = (date) => {
                const currentQuarter = getCurrentQuarter(date);
                const nextQuarter = currentQuarter === 4 ? 1 : currentQuarter + 1;
                const year = currentQuarter === 4 ? date.getFullYear() + 1 : date.getFullYear();
...
export function registerBusinessTools(server) {
    server.tool("searchProductList", "Search for products by name, SKU, or description. Use this for general product searches or when looking for multiple products. Returns a list of matching products that is to be shown to use if asked for list of products.", {
    ...
    server.tool("searchEstimateList", "Get list of estimates with optional filtering by date range and search term", {
        ...
        try {
            ...
            const estimateArray = estimates.map((estimate) => ({
    ...
    server.tool("searchInvoiceList", "Get list of invoices with optional filtering by date range and search term. When user asks for 'all invoices', 'complete list', 'full data', or similar requests, set get_all to true to fetch all records in a single request.", {
        from_date: z.string().optional().describe("Start date filter (YYYY-MM-DD format)"),
        to_date: z.string().optional().describe("End date filter (YYYY-MM-DD format)"),
        ...
        try {
            ...
            const invoiceArray = invoices.map((invoice) => ({
    ...
    server.tool("searchCustomerList", "Get list of customers with optional search filtering", {
        ...
        try {
            ...
            const customerArray = normalizedCustomers.map((customer) => ({
    ...
    server.tool("findCustomerByName", "Find a specific customer by exact name match", {
  ...
  ${lines}`;
    ...
    server.tool("getEstimateByCustomNumber", "Get detailed estimate information using customer-facing estimate number (e.g., '25-3454'). This tool automatically searches for the custom number and retrieves full details.", {
        ...
        try {
            ...
            const matchingEstimate = estimates.find((est) => [est.estimate_number, est.quotation_number, est.custom_number, est.custom_quotation_number, est.ext_po_number]
                .map(val => String(val))
            ...
            const products = Array.isArray(result.quotation_products) ? result.quotation_products.map((product) => ({
    ...
    server.tool("getInvoiceByCustomNumber", "Get detailed invoice information using customer-facing invoice number. This tool automatically searches for the custom number and retrieves full details.", {
        ...
        try {
            ...
            const matchingInvoice = invoices.find((inv) => [inv.invoice_number, inv.custom_number, inv.ext_po_number]
                .map(val => String(val))
            ...
            // Get all products with their details
            const products = Array.isArray(result.invoice_products) ? result.invoice_products.map((product) => ({
                name: product.product_title,
                description: product.product_desc || product.company_service_details?.company_service_description,
                quantity: product.product_qty,
                unit_price: parseFloat(product.company_service_details?.product_price || 0).toFixed(2),
    ...
    server.tool("getWarehouseList", "Get list of warehouses with optional filtering for warehouse management and serial number tracking", {
    ...
    // Get Stock List Tool
    server.tool("getStockList", "Get list of stock items from a specific warehouse with optional filtering and search capabilities", {
    ...
    }, async ({ warehouse_id, search, from_date, to_date, page, take, only_available, timezone, filter }) => {
        try {
            // Enhanced date cleaning and validation
            const cleanDate = (date) => {
                if (!date)
    ...
    // Delivery Board Tool
    server.tool("getDeliveryBoard", "Show all scheduled deliveries for a company or user. Use this tool to view and track what needs to be delivered and when, making it easy to plan and manage deliveries. Defaults to the current week if no date is provided. Useful for delivery planning, logistics, and daily delivery tracking.", {
    ...
    // get task list tool
    server.tool("getTaskList", "Get a list of tasks with optional filtering and pagination parameters. Use employee_name to filter tasks assigned to a specific employee (client-side filtering from all company tasks). For large datasets, use pagination to avoid response size limits.", {
        ...
        try {
            ...
            if (show_all && employee_name) {
                ...
                allTasks = allTasks.filter((task) => task.employee_name && task.employee_name.toLowerCase().includes(employee_name.toLowerCase()));
            }
            else {
            ...
            const formattedTasks = allTasks.map((task) => {
                const associatesInfo = task.associates && task.associates.length > 0
                    ? task.associates.map((assoc) => `${assoc.linked_object_name} (${assoc.custom_number})`).join(', ')
                    : 'None';
                return {
    ...
    server.tool("getNextPageTasks", "Get the next page of task list results. Use this when you want to see more tasks after viewing a paginated result.", {
        object_name: z.string().optional().describe("Object name for filtering"),
        object_id: z.string().optional().describe("Object ID for filtering"),
        filter: z.string().optional().describe("Filter criteria"),
        employee_name: z.string().optional().describe("Employee name to filter tasks assigned to specific employee"),
        ...
        try {
            ...
            // Filter by employee name if specified
            if (employee_name) {
                tasks = tasks.filter((task) => task.employee_name && task.employee_name.toLowerCase().includes(employee_name.toLowerCase()));
            }
            // Create a more readable format that highlights associates
    ...
    server.tool("getTaskDetails", "Get detailed information about a specific task using task ID. Returns comprehensive task information including description, assignee, due dates, status, and associated objects.", {
...
const ANALYTICS_UTILITIES = {
    ...
    analyzeEstimateStatuses: (estimates) => {
        const statusCounts = {};
    ...
    analyzeInvoiceStatuses: (invoices) => {
        const statusCounts = {};
        const detailedStatuses = {
            open: 0,
            created: 0,
    ...
    calculateTotals: (items) => {
        ...
        for (const item of items) {
            let val = 0;
            if (item.totals && item.totals.grand_total)
                val = parseFloat(item.totals.grand_total);
            else if (item.grand_total)
                val = parseFloat(item.grand_total);
            else if (item.quotation_total)
                val = parseFloat(item.quotation_total);
            if (!isNaN(val))
                totalAmount += val;
        }
        return totalAmount;
    ...
    generateDetailedSummary: (analysisType, entityName, estimates, invoices, dateRange) => {
        ...
        if (estimates.length > 0) {
            ...
            if (estimateAnalysis.summary.open > 0) {
                ...
                const openStatuses = [];
                if (estimateAnalysis.detailedStatuses.draft > 0)
                    openStatuses.push(`  • Draft: ${estimateAnalysis.detailedStatuses.draft}`);
                if (estimateAnalysis.detailedStatuses.sent > 0)
                    openStatuses.push(`  • Sent: ${estimateAnalysis.detailedStatuses.sent}`);
                if (estimateAnalysis.detailedStatuses.sentOpen > 0)
                    openStatuses.push(`  • Sent(Open): ${estimateAnalysis.detailedStatuses.sentOpen}`);
                if (estimateAnalysis.detailedStatuses.changeRequest > 0)
                    openStatuses.push(`  • Change Request: ${estimateAnalysis.detailedStatuses.changeRequest}`);
                if (estimateAnalysis.detailedStatuses.changeRequestUpdated > 0)
                    openStatuses.push(`  • Change Request Updated: ${estimateAnalysis.detailedStatuses.changeRequestUpdated}`);
                if (estimateAnalysis.detailedStatuses.open > 0)
                    openStatuses.push(`  • Open: ${estimateAnalysis.detailedStatuses.open}`);
            ...
            if (estimateAnalysis.summary.closed > 0) {
                ...
                if (estimateAnalysis.detailedStatuses.accepted > 0)
                    closedStatuses.push(`  • Accepted: ${estimateAnalysis.detailedStatuses.accepted}`);
                if (estimateAnalysis.detailedStatuses.closed > 0)
                    closedStatuses.push(`  • Closed: ${estimateAnalysis.detailedStatuses.closed}`);
            ...
            if (otherEstimates > 0) {
                ...
                const otherStatuses = [];
                if (estimateAnalysis.detailedStatuses.rejected > 0)
                    otherStatuses.push(`  • Rejected: ${estimateAnalysis.detailedStatuses.rejected}`);
                if (estimateAnalysis.detailedStatuses.cancelled > 0)
                    otherStatuses.push(`  • Cancelled: ${estimateAnalysis.detailedStatuses.cancelled}`);
        ...
        if (invoices.length > 0) {
            ...
            if (invoiceAnalysis.summary.open > 0) {
                ...
                const openStatuses = [];
                if (invoiceAnalysis.detailedStatuses.open > 0)
                    openStatuses.push(`  • Open: ${invoiceAnalysis.detailedStatuses.open}`);
                if (invoiceAnalysis.detailedStatuses.created > 0)
                    openStatuses.push(`  • Created: ${invoiceAnalysis.detailedStatuses.created}`);
                if (invoiceAnalysis.detailedStatuses.sent > 0)
                    openStatuses.push(`  • Sent: ${invoiceAnalysis.detailedStatuses.sent}`);
                if (invoiceAnalysis.detailedStatuses.sentOpen > 0)
                    openStatuses.push(`  • Sent(Open): ${invoiceAnalysis.detailedStatuses.sentOpen}`);
                if (invoiceAnalysis.detailedStatuses.due > 0)
                    openStatuses.push(`  • Due: ${invoiceAnalysis.detailedStatuses.due}`);
                summary += openStatuses.join('\n') + '\n';
            }
            ...
            if (invoiceAnalysis.summary.paid > 0) {
                ...
                const paidStatuses = [];
                if (invoiceAnalysis.detailedStatuses.paid > 0)
                    paidStatuses.push(`  • Paid: ${invoiceAnalysis.detailedStatuses.paid}`);
                if (invoiceAnalysis.detailedStatuses.closed > 0)
                    paidStatuses.push(`  • Closed: ${invoiceAnalysis.detailedStatuses.closed}`);
    ...
    server.tool("analyzeBusinessData", "Analyze business data and return chart-ready JSON for infographics. Supports company-wide sales analytics, customer-specific analysis, and employee-based analysis. IMPORTANT: This tool only accepts exact dates in YYYY-MM-DD format. For natural language dates like 'this month', 'last quarter', etc., use the date-utility tool first to convert them to exact dates.", {
        ...
        try {
            ...
            const validateDateFormat = (dateStr) => {
                if (!dateStr)
                    return true; // Optional dates are fine
            ...
            if (analysis_type === "total_sale_for_customer") {
                ...
                if (estimateStatuses.length > 1 && customerAnalyticsData.metrics.estimates.total > 0) {
                    customerCharts.estimate_status = {
                        type: "pie",
                        data: {
                            labels: estimateStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${customerAnalyticsData.metrics.estimates.statusBreakdown[status]})`),
                            datasets: [{
                ...
                if (invoiceStatuses.length > 1 && customerAnalyticsData.metrics.invoices.total > 0) {
                    customerCharts.invoice_status = {
                        type: "pie",
                        data: {
                            labels: invoiceStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${customerAnalyticsData.metrics.invoices.statusBreakdown[status]})`),
                            datasets: [{
                ...
                estimates.forEach((est) => {
                    ...
                    let val = 0;
                    if (est.totals && est.totals.grand_total)
                        val = parseFloat(est.totals.grand_total);
                    else if (est.grand_total)
                        val = parseFloat(est.grand_total);
                    else if (est.quotation_total)
                        val = parseFloat(est.quotation_total);
                    if (!isNaN(val))
                        monthlyData[monthKey].estimateAmount += val;
                });
                invoices.forEach((inv) => {
                    ...
                    let val = 0;
                    if (inv.totals && inv.totals.grand_total)
                        val = parseFloat(inv.totals.grand_total);
                    else if (inv.grand_total)
                        val = parseFloat(inv.grand_total);
                    if (!isNaN(val))
                        monthlyData[monthKey].invoiceAmount += val;
                });
                ...
                if (estimateStatuses.length > 1 && analyticsData.metrics.estimates.total > 0) {
                    charts.estimate_status = {
                        type: "pie",
                        data: {
                            labels: estimateStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${analyticsData.metrics.estimates.statusBreakdown[status]})`),
                            datasets: [{
                ...
                if (invoiceStatuses.length > 1 && analyticsData.metrics.invoices.total > 0) {
                    charts.invoice_status = {
                        type: "pie",
                        data: {
                            labels: invoiceStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${analyticsData.metrics.invoices.statusBreakdown[status]})`),
                            datasets: [{
                ...
                const employeeCustomers = new Set();
                estimates.forEach((est) => {
                    if (est.customer_name)
                        employeeCustomers.add(est.customer_name);
                });
                invoices.forEach((inv) => {
                    if (inv.customer_name)
                        employeeCustomers.add(inv.customer_name);
                });
                ...
                if (estimateStatuses.length > 1 && employeeAnalyticsData.metrics.estimates.total > 0) {
                    employeeCharts.estimate_status = {
                        type: "pie",
                        data: {
                            labels: estimateStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${employeeAnalyticsData.metrics.estimates.statusBreakdown[status]})`),
                            datasets: [{
                ...
                if (invoiceStatuses.length > 1 && employeeAnalyticsData.metrics.invoices.total > 0) {
                    employeeCharts.invoice_status = {
                        type: "pie",
                        data: {
                            labels: invoiceStatuses.map(status => `${status.charAt(0).toUpperCase() + status.slice(1)} (${employeeAnalyticsData.metrics.invoices.statusBreakdown[status]})`),
                            datasets: [{
            ...
            if (analysis_type === "employee_comparison_analytics") {
                ...
                for (const employeeName of employee_names) {
                    ...
                    const employeeEstimates = allEstimates.filter((estimate) => {
                    ...
                    const employeeInvoices = allInvoices.filter((invoice) => {
                    ...
                    const analysisResult = ANALYTICS_UTILITIES.generateDetailedSummary("Employee Sales Analytics", employeeName, employeeEstimates, employeeInvoices, `from ${parsedFromDate} to ${parsedToDate} (inclusive)`);
